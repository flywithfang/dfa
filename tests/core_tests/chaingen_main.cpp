...
int main(int argc, char* argv[])
{
  ...
  if (command_line::get_arg(vm, arg_generate_test_data))
  {
     ...
     std::vector<test_event_entry> events; 
        gen_simple_chain_001 g; 
        g.generate(events); 
        auto filename= "chain001.dat";
  ...
  {
    auto filename= "chain001.dat";
      if(do_replay_file<gen_simple_chain_001>(filename)) 
    { 
      MERROR("Failed to pass test : " << "gen_simple_chain_001"); 
      return 1; 
    }
  }
